...
export function createTeamSelect({ rosters, onConfirm, onBack, onBrowse }) {
  ...
  let teamIdx = 0;
  let playerIdx = 0;
  let mode = 'match'; // 'match' picks a team; 'derby' picks one slugger

  root.innerHTML =
    `<div class="screen-title" id="ts-title">CHOOSE YOUR MUTANTS</div>` +
  ...
  const titleEl = root.querySelector('#ts-title');
  ...
  confirmEl.addEventListener('click', () => onConfirm(teamKeys[teamIdx], playerIdx));
  ...
  function onKey(e) {
    ...
    else if (e.code === 'Enter') onConfirm(teamKeys[teamIdx], playerIdx);
  ...
  return {
    show(m = 'match') { mode = m; visible = true; root.classList.remove('hidden'); render(); },
